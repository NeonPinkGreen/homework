...
class User{
	...
	render(){
		return `<div class="user">
			...
			</div>${this.courses ? this.renderCourses() : ``}
        </div>`;
	...
	renderCourses(){
		...
		return `<div class="user__courses">${coursesRender}</div>`;
	}
}
...
class Lector extends User{
	...
	renderCourses(){
		...
		return `<div class="user__courses admin--info">${coursesRender}</div>`;
	}
}
...
let usersClass = users
	.map(user => USERS_ROLES[user.role] ? USERS_ROLES[user.role](user) : new User(user))
